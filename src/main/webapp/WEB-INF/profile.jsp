...
<div class="container">
    ...
    <div>
        <h1> Your Ads: </h1>
        <c:forEach items="${sessionScope.userAds}" var="ad">
            <div>
                <h2> <c:out value="${ad.title}"/> </h2>
                <p> <c:out value="${ad.description}"/> </p>
                <form class= "ad_btn" action="/ad/page" method="get">
                    <button class="ad_btn" type="submit" name="title_of_ad" value="${ad.title}"> View Ad </button>
                </form>
                <form class= "ad_btn" action="/delete" method="post">
                    <button type="submit" name="delete_this_ad" value="${ad.title}">Delete Ad</button>
                </form>
            </div>
        </c:forEach>
    </div>
